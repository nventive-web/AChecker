	...
	<fieldset class="group_form"><legend class="group_form"><?php echo _AC("input"); ?></legend>
		...
		<div class="topnavlistcontainer"><br />
			<ul class="navigation">
				...
				<li class="navigation"><a href="javascript:void(0)" accesskey="c" title="TotalValidator Alt+4" id="AC_menu_from_totalvalidator" onclick="return AChecker.input.onClickTab('AC_from_totalvalidator');" <?php if (isset($_POST["validate_tv"])) echo 'class="active"'; ?>><span class="nav">TotalValidator</span></a></li>
			</ul>
		</div>
		...
		<div id="AC_from_totalvalidator" class="input_tab" style="<?php if (isset($_POST["validate_tv"])) echo "display:block"; else echo "display:none"; ?>">
			<div style="text-align:center;">
				<label for="checktv"><?php echo _AC('file'); ?>:</label>
				<input type="hidden" name="MAX_FILE_SIZE" value="52428800" />
				<input type="file" id="checktv" name="uploadtv" size="47" />
				...
				<div class="validation_submit_div">
					...
					<input class="validation_button" type="submit" name="validate_tv" id="validate_tv" value="<?php echo _AC("check_it"); ?>" onclick="return AChecker.input.validateUpload('checktv');"  />
				</div>
			</div>
		</div>
